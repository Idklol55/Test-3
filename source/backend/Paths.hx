...
class Paths
{
	...
	static public function image(key:String, ?library:String = null, ?allowGPU:Bool = true):FlxGraphic
	{
		...
		else if (PsychFileSystem.exists(file))
	...
	static public function getTextFromFile(key:String, ?ignoreMods:Bool = false):String
	{
		...
		if(PsychFileSystem.exists(path)) return PsychFile.getContent(path);
		return null;
	}
	...
	public static function fileExists(key:String, type:AssetType, ?ignoreMods:Bool = false, ?library:String = null)
	{
		...
		if(!ignoreMods)
		{
			for(mod in Mods.getGlobalMods())
				if (PsychFileSystem.exists(mods('$mod/$key')))
					return true;
				#if linux
				else if (PsychFileSystem.exists(findFile('$mod/$key')))
					return true;
				#end

			if (PsychFileSystem.exists(mods(Mods.currentModDirectory + '/' + key)) || PsychFileSystem.exists(mods(key)))
				return true;
			
			if (PsychFileSystem.exists(mods('$key')))
				return true;
			#if linux
			else if (PsychFileSystem.exists(findFile(key)))
				return true;
			#end
		}
		#end
	...
	inline static public function getSparrowAtlas(key:String, ?library:String = null, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		return FlxAtlasFrames.fromSparrow(imageLoaded, (xmlExists ? PsychFile.getContent(xml) : getPath('images/$key.xml', library)));
		#else
	...
	inline static public function getPackerAtlas(key:String, ?library:String = null, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		return FlxAtlasFrames.fromSpriteSheetPacker(imageLoaded, (txtExists ? PsychFile.getContent(txt) : getPath('images/$key.txt', library)));
		#else
	...
	inline static public function getAsepriteAtlas(key:String, ?library:String = null, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		return FlxAtlasFrames.fromTexturePackerJson(imageLoaded, (jsonExists ? PsychFile.getContent(json) : getPath('images/$key.json', library)));
		#else
	...
	/*private static function getContentFromFile(path:String):String
	{
		...
		if(PsychFileSystem.exists(path) || (onAssets = true && Assets.exists(path, TEXT)))
		{
			//trace('Found text: $path');
			return !onAssets ? PsychFile.getContent(path) : Assets.getText(path);
		}
		return null;
	}*/
	#end

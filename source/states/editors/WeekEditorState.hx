...
class WeekEditorState extends MusicBeatState
{
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED PsychFileSystem.exists(Paths.modsImages('menubackgrounds/menu_' + assetName)) || #end
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED PsychFileSystem.exists(Paths.modsImages('storymenu/' + assetName)) || #end
	...
	private static function onLoadComplete(_):Void
	{
		...
		if(fullPath != null) {
			var rawJson:String = PsychFile.getContent(fullPath);
			if(rawJson != null) {

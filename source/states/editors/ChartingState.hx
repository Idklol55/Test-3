	...
	**/
	...
	function addSongUI():Void
	{
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			#if MODS_ALLOWED
			if (#if MODS_ALLOWED PsychFileSystem.exists(Paths.modsJson(songName + '/events')) || #end PsychFileSystem.exists(file))
			#else
		...
		for (i in 0...directories.length) {
			...
			//if(PsychFileSystem.exists(directory)) {
				for (file in PsychFileSystem.readDirectory(directory)) {
					...
					if (!PsychFileSystem.isDirectory(path) && file.endsWith('.json')) {
	...
	function addEventsUI():Void
	{
		...
		for (i in 0...directories.length) {
			...
			//if(PsychFileSystem.exists(directory)) {
				for (file in PsychFileSystem.readDirectory(directory)) {
					var path = haxe.io.Path.join([directory, file]);
					if (!PsychFileSystem.isDirectory(path) && file != 'readme.txt' && file.endsWith('.txt')) {
						var fileToCheck:String = file.substr(0, file.length - 4);
						if(!eventPushedMap.exists(fileToCheck)) {
							eventPushedMap.set(fileToCheck, true);
							eventStuff.push([fileToCheck, PsychFile.getContent(path)]);
						}
					}
				}
			//}
	...
	function loadCharacterFile(char:String):CharacterFile {
		...
		var rawJson = PsychFile.getContent(path);
